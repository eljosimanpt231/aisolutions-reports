...
- `'content_generation'` — **Memo.ria** (Centro Memoria; clínica multidisciplinar Aveiro). **Sem chatbot** — geração diária de blog posts SEO. `services: ['content']`, renderizado por `renderContentSection` (separado de `renderChatbotSection`). KPIs: Posts Gerados, Publicados + taxa, Pilares Cobertos, Fotos Disponíveis (semáforo: verde>30, laranja 10-30, vermelho<10). Charts: produção diária (bar), donut pilares, donut status. Tabelas: últimos 10 posts (link Webflow quando publicado), cobertura keywords com destaque para as nunca usadas
...
- **v=36** — Isabel Pedroso: origem das leads + reservas pagas + fix canal.
  ...
  - Frontend backward-compatible (guards) — blocos novos só renderizam quando o backend devolver `origens`/`pagamentos`/`confirmadas`. Backend aplicado a 06/08 (patch `tmp_explore/patch_ips_origens.js`, PUT 200 + re-GET OK; backup `tmp_explore/backup-wf-dashboards-pre-ips-origens-2026-08-06.json`); webhook E2E validado.
  - **Filtros de dados de teste (patch v2, 06/08 — `tmp_explore/patch_ips_testdata_filter.js`):** `automsgs_log` filtra `test_mode IS NOT TRUE` (lembrete_prestacoes era 105/105 teste → desaparece da tabela); `ghl_sync` e `chat_histories` excluem os 9 números da equipa/Josias (`IPS_TEAM` no Build Queries) em conversas, hourly, funil, autonomia e origens. **Decisão: filtrar em query, NUNCA apagar da BD** — `automsgs_log` é estado de dedup das automações (apagar → reenvios) e `ghl_sync` é sync do GHL (apagar → recriado). `payment_links` já filtrava `is_test`.
- **v=34** — 4 novos dashboards em produção (contexts próprios):
  ...
  - **Translowcost** (`qualificador_mudancas`): agente Bia em 2 instâncias WA (`wp_chat_histories` + `_wa2` via UNION ALL). Sync PipeDrive via `client_state.current_stage`. Funil 4 stages + donuts tipo/idioma/origem.
  - **Memo.ria** (`content_generation`): PRIMEIRO cliente de conteúdo/SEO. Novo `services: ['content']` — sem chatbot. Nova `renderContentSection` + `initContentCharts` + `getContentMetrics`. Fonte: `blog_posts`, `pillars_keywords`, `stock_photos`. KPIs: posts gerados, publicados, taxa publicação, pilares cobertos, fotos disponíveis (semáforo). Tabelas: últimos posts + cobertura keywords. Combine Results expõe extended_data como `data.content` quando `config.contextType === 'content_generation'`.
