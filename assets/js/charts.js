...
function initMessagingCharts(data) {
  ...
  // Labels legíveis (MBWAY -> MBWay, códigos operacionais da Núbia) com o mesmo mapa da tabela
  const lab = (t) => (typeof prettyMsgType === 'function' ? prettyMsgType(t) : t);
  (data.operacionais || []).forEach((r, i) => {
    labels.push(lab(r.tipo));
    items.push(r.total);
